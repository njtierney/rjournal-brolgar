...
```{r heights-example-three-features, echo = TRUE}
...
p1 <- gg_feat(heights_three, min) 
p2 <- gg_feat(heights_three, median)
p3 <- gg_feat(heights_three, max)
library(patchwork)
p1 + p2 + p3
```
...
```{r features-monotonic, echo = TRUE}
...
> Perhaps define a function to make it clear you can say for example identify the, say, second element of a series.
...
Plots like Figure \@ref(fig:spaghetti) are often called, "spaghetti plots", and can be useful for a high level understanding as a whole. However, we cannot process and understand the individuals.

## Sampling

Just how spaghetti is portioned out for consumption, it is useful to randomly portion out the data into sub-plots, to tease out some of the data (Figure \@ref(fig:facet-sample)). 
...
Plot ... ach key is shown only once, and is randomly assigned to a facet/sub-plot. We can provide meaning to each of these facets, by arranging the heights "along" a variable, like `year`:
...
> "Who is stereotypical of the minimum"
...
```{r heights-keys-near-fancy-label}
heights_near_aug <- keys_near(heights_aug, 
          var = res,
          top_n = 5) %>% 
  left_join(heights_aug,
