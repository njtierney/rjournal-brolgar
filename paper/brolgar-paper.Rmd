...
```{r source-pkgs, echo = FALSE, include = FALSE}
suppressMessages(source(here::here("packages.R")))
suppressMessages(source(here::here("conflicts.R")))
```
...
```{r heights-sample-plot, fig.cap = "A sample of average heights of men in Australia for 1900-1970. The heights increase over time, but are measured at irregular intervals.", echo = FALSE, fig.height = 5, fig.width = 12}
...
This paper proceeds in the following way: first, a brief review of existing approaches to longitudinal data (Section \@ref(other-approaches)), then the definition of longitudinal data (Section \@ref(data)), then approaches to these four problems are discussed (Section \@ref(building-on-tsibble) - Section \@ref(steretyping)), followed by a summary in Section \@ref(summary).

# Background {#other-approaches}

R provides basic time series, `ts`, objects, which are vectors or matrices that represent data sampled at equally spaced points in time. These have been extended through packages such as `xts`, and `zoo` [@xts; @zoo], which only consider data in a wide format with a regular implied time series. These are not appropriate for longitudinal data, which can have indexes that are not time unit oriented, such as "Wave 1...2", or may contain irregular intervals.
...
## Building on `tsibble` {#building-on-tsibble}
...
For more information run `library(feasts)` then `?fabletools::feature_set`, to see other features available in the `feasts` R package.
...
Extending this idea of samples, we can instead look at **all** of the data, spread out equally over facets, using `facet_strata()`. Is uses 12 facets by default, controllable with `n_strata`. The code to do so is shown below, creating Figure \@ref(fig:facet-strata).
...
Figure \@ref(fig:facet-strata) and Figure \@ref(fig:facet-sample) only show each key once, being randomly assigned to a facet. We can meaningfully place the keys into facets, by arranging the heights "along" a variable, like `year`, using the `along` argument in `facet_strata` to produce Figure \@ref(fig:facet-year-along):
...
```{r feature-first-gg, fig.cap = "Distribution of starting years of measurement. The data is already binned into 10 year blocks. Most of the years start between 1840 and 1900."}
heights_brolgar %>% 
  features(year, c(first = first)) %>% 
  ggplot(aes(x = first)) +
  geom_bar()
...
```{r heights-max-in-examine, fig.cap = "The different distributions of the features - A showing the distribution of maximum height, and B showing the number of countries that are always increasing (FALSE), and always increasing (TRUE). We note that the average maximum heights range from about 160cm to 185cm, with most being around 170cm. We also learn that vast majority of countries are not always increasing in height", echo = FALSE, fig.height = 4}
...
We can then display the data by highlighting it in the background, first creating a background plot and overlaying the plots on top of this as an additional ggplot layer, in Figure \@ref(fig:gg-increase-max).
...
```{r gg-increase-max, echo = FALSE, fig.cap = "Plots of the data in background, with the countries that always increase in height in A, and the country with the tallest people on B", fig.height = 4}
...
These same workflows can be used to interpret and explore a model. As the data tends to follow a non linear trajectory, we use a general additive model (gam) with the `mgcv` R package [@mgcv] using the code below:
...
```{r sample-n-keys-model, fig.cap = "Exploration of a random sample of the data using the 'sample_n_keys' function. This shows the data in points of 12 countries, with the model fit (blue)."}
heights_aug %>% 
  sample_n_keys(12) %>% 
  ggplot(aes(x = year,
             y = pred,
...
```{r heights-aug-res-summary, fig.cap = "Five number summary of residual values from the model fit. The residuals are centered around zero with some variation.", echo = FALSE}
...
```{r heights-keys-near, fig.cap = "The keys nearest to the five number summary of the residual sums of squares are displayed. We learn that Moldova and Madagascar are fit by a nearly straight line, and the remaining countries with poorer fit have greater variation in height.", fig.height = 4}
...
```{r heights-keys-near-fancy-label, fig.cap = "Figure of stereotypes for three", echo = FALSE}
