...
spaghetti <- ggplot(heights_prep,
       aes(x = year,
           y = height_cm,
           group = country)) + 
  geom_line()

spaghetti_alpha <- ggplot(heights_prep,
       aes(x = year,
           y = height_cm,
           group = country)) + 
  geom_line(alpha = 0.3)
...
heights_prep <- as_tsibble(heights_prep,
                      index = year,
                      key = country,
                      regular = FALSE)
...
```{r heights-summary}
heights_min <- features(.tbl = heights_prep, 
                        .var = height_cm, 
                        features = c(min = min))
...
This fits height in centimetres with a smooth effect for year for each country, with a different intercept for each country. It is roughly equivalent to a random intercept varying slope model. Note that this gam model took approximately `r model_time` seconds to fit. We add the predicted and residual values for the model below
