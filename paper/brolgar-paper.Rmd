...
# troubleshooting
  # could be some issue to do with the start date that isn't the same
  # i.e., the Loblolly data all IDs start at the same times
  # using something like the broken stick model
  # a bunch of different loess models could also be possible

heights_nlme <- heights  %>%
  mutate(year0 = year - 1710)

library(nlme)
heights_nlme_fit <- nlme(
  # model = height_cm ~ year,
  height_cm ~ SSasymp(year0, Asym, R0, lrc),
  data = heights_nlme,
  fixed = Asym + R0 + lrc ~ 1,
  random = Asym ~ 1,
  # fixed = year0 ~ 1,
...
```{r}
heights_nlme_test <- heights_nlme %>% 
  filter(n_obs > 15)

ggplot(heights_nlme_test,
       aes(x = year,
           y = height_cm,
           group = country)) + 
  geom_line() + 
...
getInitial(height_cm ~ SSasymp(year0, Asym, resp0, lrc), 
           data = heights_nlme_test)
```
...
```{r heights-lmer}
heights_prep <- heights %>% mutate(year0 = year - 1710)
```


```{r}
library(mgcv)
gam_heights <- heights_prep %>% 
...
```{r heights-lmer}
heights_fit <- lmer(height_cm ~ I(year0^2) + (1|country), data = heights_prep)
heights_aug <- heights_prep %>%
