...
`brolgar` has some pre-specified features, the five number summary (minimum, 25th quantile, median, mean, 75th quantile, and maximum) of the data can be calculated with `feat_five_num`:
...
This takes the `heights` data, piping it to `features`, and then telling it to summarise the `height_cm` variable, using `feat_five_num`. Other features built into `brolgar` include:
...
## Exploring data using intercept and slope

Another feature that can be used to explore the data is to fit a linear model for each `key` in a dataset. This approach is discussed in more detail in chapter 25 of R4DS [@r4ds]. `brolgar` provides a helper function to help with this, called `key_slope()`. 
...
We can explore the features of the slope of the data

... exploring intercept

... exploring slope

We can then join these summaries back to the data like so:
...
And highlight those individuals like so:
...
We need a method to identify how these slopes are similar to each other.
