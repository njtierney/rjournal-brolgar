...
### Feature Sets (note - potentially remove? This isn't something we really demonstrate in the paper)
...
```{r gg-increase-max, echo = FALSE, fig.cap = "Plots of the data in background, with the countries that always increase in height in A, and the country with the tallest people on B"}
...
**note - try using RSS instead of only residuals for the keys_near**


**note - also look into Sweden - it's there twice**

- Sweden appears only once in the data
...
```{r heights-keys-near, fig.cap = "keys nearest visualised"}
...
We can also look at the nearest 3 countries to each statistic, by changing `top_n = 3`:
...
# How to Install and Getting Started {#install}
